...
// view for individual recipe, containing multiple products

if (!empty($_GET['id'])) {
    $get_parameter = $_GET['id'];
	...
	$read_query = "SELECT recipes.recipes.recipe_name, recipes.products.product_name, recipes.recipes_products.product_quantity 
                   ...
                   LEFT JOIN recipes.recipes_products ON recipes.recipe_id = recipes_products.recipe_id 
                   LEFT JOIN recipes.products ON recipes_products.product_id = products.product_id 
                   WHERE recipes.recipe_id = '$get_parameter'";
	$result = mysqli_query($connection, $read_query);

	// var_dump(mysqli_fetch_all($result, MYSQLI_ASSOC));
	// die();

    // if any products, iterate and print
