...
$read_query = "SELECT p.`product_id`, p.`product_name`, p.`product_price`, p.`product_calories`, pc.product_category_name FROM `products` p LEFT JOIN product_categories pc ON p.`product_category_id`=pc.product_category_id WHERE p.date_deleted IS NULL $filter_query";
